...
```
Phase 0  Bootstrap*      domain-design (reverse)  อ่าน codebase เก่า → design/ + reverse-notes.md (ยังไม่มี scenarios.json)
...
```
\* Phase 0 รันเฉพาะตอนที่ **ยังไม่มี `scenarios.json`** และ target ที่สั่งเป็น **codebase เดิม** (ดู 2D)
ไม่งั้น orchestrator จะหยุดแล้วบอกให้รัน scenario-discovery ก่อนเหมือนเดิม
...
### D. มี codebase เก่า (brownfield) — ยังไม่มี scenarios.json เลย

สั่งตรงๆ ผ่าน orchestrator ได้เลย ไม่ต้องเรียก domain-design reverse mode เองแยกต่างหากอีกต่อไป:
```
/orchestrator:build D:\GitHub\LegacyApp
```
orchestrator เห็นว่า `scenarios.json` ยังไม่มี **และ** target เป็น path ของ codebase จริง (ไม่ใช่ชื่อ
module เฉยๆ) → จะแทรก **Phase 0 (bootstrap)** เข้าไปก่อน Phase 1 อัตโนมัติ:

```
Phase 0   domain-design (reverse) → design/ + reverse-notes.md (ไม่มี scenarios.json ตอนนี้ ไม่เขียน traces_down)
Phase 1   scenario-discovery      → ถามยืนยัน business intent ทีละ candidate จาก reverse-notes.md (ไม่เดาให้)
Phase 2   domain-design (UPDATE)  → ผูก traces_down เข้ากับ entity/API ที่สกัดไว้แล้วจาก Phase 0 (ไม่อ่านโค้ดซ้ำ)
... ต่อด้วย 2u/3/4/4q ตามปกติ
```

ดูแผนก่อนได้เหมือนเดิม:
```
/orchestrator:plan D:\GitHub\LegacyApp
```
(ตอนนี้ `scope.ids` ยังว่างเพราะยังไม่รู้จนกว่า Phase 1 จะยืนยันเสร็จ)

**ข้อควรรู้:**
- Phase 0 **ไม่เดา business intent ให้** — แค่ดึงโครงสร้าง (entities/API/sitemap) จากโค้ด แล้วส่งเป็น
  "candidate ที่ต้องให้คนยืนยัน" ต่อใน Phase 1 เสมอ (กันไม่ให้ AI สร้าง requirement เท็จจากโค้ดเก่า)
- ถ้าอยากได้แค่เอกสาร (ไม่ได้จะพัฒนาต่อทันที) — ดูที่ `domain-design` USER-GUIDE หัวข้อ 3.4.1 แทน
  (เรียก reverse mode + `/deliver-docs` + word-export ตรงๆ โดยไม่ผ่าน orchestrator ก็ได้ ถ้าไม่ต้องการ
  ให้ orchestrator ไล่ทำ Phase 2u-4q ต่อจนถึงโค้ดจริง)
- รองรับเต็มที่เฉพาะ ASP.NET Core + EF Core ตอนนี้ (ดู `domain-design/references/codebase-analysis.md`)
...
`scope` = ชื่อ module / `SC-id` ตัวเดียว / path codebase เดิม (ถ้ายังไม่มี `scenarios.json` — ดู 2D) /
เว้นว่าง (= module ปัจจุบันใน `scenarios.json#meta.module`)
...
|---|---|---|
| 0-reverse | domain-design (reverse) | design/ + reverse-notes.md มีจริง + **ยังไม่มี** scenarios.json ถูกสร้าง (เฉพาะ brownfield bootstrap) |
...
run เต็มสายมี 6 phase (7 ถ้าเป็น brownfield ที่มี Phase 0 bootstrap) ใช้เวลานาน session อาจจบกลางคัน orchestrator เก็บสถานะใน
...
- domain-design มี `/domain-design:deliver-docs`; reverse mode (Phase 0/2) และ word-export ยังไม่มี `/`
  command เป็นทางการ — orchestrator/คุณเรียกผ่าน implicit skill invoke ("reverse-engineer codebase ที่...")
