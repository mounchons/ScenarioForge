...
Do this:
1. Read `qa-tracker.json`. If it doesn't exist, tell the user to run `/scenario-verify:generate` then
   `/scenario-verify:run`.
...
3. Re-dispatch each batch at its recorded tier (4-part contract) and continue the run loop exactly as
   `/run` does: fix the test on a test defect, file a finding on a real app defect, respect the caps,
   escalate a half-cap Haiku/Sonnet scenario to Opus once.
4. Recompute `coverage` + `rollup`, enforce **Gate 4**, end with the Handoff block.
