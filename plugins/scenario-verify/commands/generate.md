---
description: Derive E2E test scenarios (TS-xxx) from acceptance criteria + UI Control Manifests into qa-tracker.json
...
2. Read `qa-tracker.json` if it exists -> detect **CREATE vs APPEND** (`references/qa-tracker.md`). In
...
3. For each control in each manifest, derive its mandatory categories per
   ...
   `permission` per role, one `validation` per rule).
...
5. Write them `pending` into `qa-tracker.json` with `control_refs`, `category`, `category_detail`,
   `manifest_path` + `manifest_emitted_at`. Recompute `coverage` + `rollup`.
6. Do **not** run them — end by reporting the generated set (counts by category + tier) and pointing the
