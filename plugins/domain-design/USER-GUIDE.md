...
`sample/doc-meta.json`. ลอง `sample/` ก่อนได้เลย (`python render_sdd_docx.py sample/system-design-document.md sample/output/out.docx --meta sample/doc-meta.json`)
...
Dictionary) — **ส่วนที่เหลือ (Screen/Process/Document/Job spec, ตาราง Server/DB/API/Security/
Performance/Reliability) ต้องกรอกเองเพราะ ScenarioForge ไม่มีข้อมูลนี้จริงๆ** (Screen เป็นของ
ui-mockup, ส่วน infra/server เป็นข้อมูลที่ต้องมาจากคนจริงเท่านั้น) — โครงสร้างตาราง/หัวข้อ
ยังอยู่ครบ พร้อมให้กรอกมือ รายละเอียดทั้งหมดอยู่ที่ `references/word-export.md`
